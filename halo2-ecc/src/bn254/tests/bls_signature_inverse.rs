...
use crate::{
    bn254::bls_signature_inverse::BlsSignatureChip2, fields::FpStrategy,
...
fn bls_signature_test<F: BigPrimeField>(
    ...
    let bls_signature_chip = BlsSignatureChip2::new(&fp_chip, &pairing_chip);
    let result = bls_signature_chip.bls_signature_verify(ctx, g2, signatures, pubkeys, msghash);

    // Calculate non-halo2 pairing by multipairing
    let mut pubkeys_g2: G2Affine = pubkeys[0];
    for pubkey in pubkeys.iter().skip(1) {
        pubkeys_g2 = (pubkeys_g2 + pubkey).into();
    }
    let pubkeys_g2_prepared = G2Prepared::from(pubkeys_g2);

    let mut signatures_g1: G1Affine = signatures[0];
    for sig in signatures.iter().skip(1) {
        signatures_g1 = (signatures_g1 + sig).into();
    }
    let signature_aggregated = signatures_g1;

    let g2_prepared = G2Prepared::from(-g2);
    let actual_result =
        multi_miller_loop(&[(&signature_aggregated, &g2_prepared), (&msghash, &pubkeys_g2_prepared)])
...
fn test_bls_signature() {
    ...
    let msg_hash = G1Affine::random(OsRng);
    // println!("hash(m):{:?}",msg_hash);
    let g2 = G2Affine::generator();
    // println!("g1:{:?}",g1);
    let mut signatures: Vec<G1Affine> = Vec::new();
    let mut pubkeys: Vec<G2Affine> = Vec::new();
    for _ in 0..params.num_aggregation {
        let sk = Fr::random(OsRng);
        // print!("{:?}",sk);
        let signature = G1Affine::from(msg_hash * sk);
        let pubkey = G2Affine::from(G2Affine::generator() * sk);
    ...
    base_test().k(params.degree).lookup_bits(params.lookup_bits).run(|ctx, range| {
        ...
        bls_signature_test(ctx, range, params, g2, &signatures, &pubkeys, msg_hash);
    })
}
...
fn bench_bls_signature() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in bench_params_reader.lines() {
        ...
        let stats = base_test().k(k).lookup_bits(bench_params.lookup_bits).bench_builder(
            (g2, signatures.clone(), pubkeys.clone(), msg_hash),
            (g2, signatures, pubkeys, msg_hash),
            |pool, range, (g2, signatures, pubkeys, msg_hash)| {
                bls_signature_test(
                    pool.main(),
                    range,
                    bench_params,
